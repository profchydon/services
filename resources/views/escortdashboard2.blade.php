...
@include('pages.escorts.dashboard2')
...
$(document).ready(function(){
    ...
    $.ajax({
        ...
        .done(function(error) {
            ...
            for (var state in mydata.States) {
                ...
                $('#edit-state').append('<option value="' + String(state) + '">' + String(state) + ' </option>');
            }
        })
    ...
    $('#edit-state').change(function(event) {
        ...
        //Get cities for selectedState
        var cities = mydata.States[String(selectedState)];
        $('#edit-city').append('<option value="" selected disabled>Please select</option>');

        for (var i = 0; i < cities.length; i++) {
            $('#edit-city').append('<option value="' + String(cities[i]) + '">' + String(cities[i]) + ' </option>');
        }
    });
